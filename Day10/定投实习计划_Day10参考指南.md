...
| 5 | 完成 Day10 报告并提交 GitHub | 填完报告模板.md + 确认月报文件，两份产出物一起 push |
...
- 复制进 `Day10/report/报告模板.md` 的**第二节**（月度汇报 → 持仓现状数字）
- 直接作为 `Day10/report/月度汇报_2026年7-8月.md` 的**第一节**，其余五节自己填写

> 如果 `月度汇报_2026年7-8月.md` 已经存在且有内容，只把 Skill 生成的第一节数字替换进去，保持其余五节不变。

两份产出物都要提交：
- **Day10 报告** = `Day10/report/报告模板.md`（填完所有 ___）
- **月度汇报** = `Day10/report/月度汇报_2026年7-8月.md`（Skill 填第一节 + 自己填其余五节）

填完检查：报告模板.md 是否还有 ___ 空缺，数字是否和 portfolio.xlsx 一致。
